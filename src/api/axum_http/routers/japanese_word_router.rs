...
use crate::{
    api::controllers::japanese_word_controller::{get_japanese_word, get_japanese_word_test},
...
pub fn routes(db_pool: Arc<PgPool>) -> Router {
    ...
    let get_japanese_word_test_use_case =
        GetJapaneseWordTestUseCase::new(Arc::new(japanese_word_repository));

    Router::new()
        .route("/{kanji}", get(get_japanese_word))
        .with_state(Arc::new(get_japanese_word_use_case))
        .route("/test/{kanji}", get(get_japanese_word_test))
        .with_state(Arc::new(get_japanese_word_test_use_case))
}
